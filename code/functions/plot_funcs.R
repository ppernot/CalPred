...
orchid_tr= col2tr("orchid",35)
blue_tr  = col2tr("blue"  ,20)
blue_tr2  = col2tr("blue"  ,40)
black_tr = col2tr("black"  ,30)
yellow_tr= col2tr("yellow"  ,120)
orange_tr= col2tr("orange"  ,80)

colors=c()
colors[2:9] = brewer.pal(8,'Set1')
colors[1] = 'black'
...
# Plot functions ####
plot_hull <- function(xlim=NULL, ylim=NULL, lwd= 6,
  ...
  if(!is.null(true_point))
    points(true_point[1],true_point[2],pch=8,cex=1.5,lwd=4,col=2)

  if(meth_type == 'hier') {
    cols= rep(colors[1:5],2)
  ...
  P=compute.bagplot(eps, sig, factor =1.96)
  poly = P$hull.loop
  polygon(poly,col=blue_tr2,border=4,lwd=3,lty=1)
  points(mean(eps),mean(sig),pch=20,cex=0.5,col=4)
  grid(); box()
...
plot_predict <- function(center=FALSE, ylim=NULL, 
  ...
  par(mfrow=c(1,1),mar=c(3,3,1.6,.2),mgp=c(2,.75,0), 
      pty='s',tcl=-0.5, cex=cexPlot)
  ylab = expression(paste('Viscosity / ',mu,'Pa.s'))
  if(center) 
  ...
  pch = c(rep(16,5),rep(17,5))
  if(center) {
    # Center on best fit on MAP
    lp  = extract(fit,"lp__")[[1]]
    map = which.max(lp)
    if(model_tag == 'HierC-loc') {
      mu = extract(fit,"mu")[[1]]
      eps = mu[map,1,]
      sig = mu[map,2,]
      c_new = c()
      for(i in 1:length(data$x_pred))
        c_new[i] = phys_mod(data$x_pred[i],eps[data$tag2[i]],
                            sig[data$tag2[i]],inadequacy)
    ...
    lines(data$x_pred,q_pred[,1]-c_new,col=4,lwd=2,lty=3)
    lines(data$x_pred,q_pred[,2]-c_new,col=4,lwd=2,lty=3)
    polygon(cbind(data$x_pred,rev(data$x_pred)),
            cbind(q_conf[,1]-c_new,rev(q_conf[,2]-c_new)),
            col=blue_tr, border=NA)
    lines(data$x_pred,q_conf[,1]-c_new,col=4,lwd=2,lty=1)
    lines(data$x_pred,q_conf[,2]-c_new,col=4,lwd=2,lty=1)
    ...
    polygon(cbind(data$x_pred,rev(data$x_pred)),
            ...
            col=blue_tr, border=NA)
    # lines(data$x_pred,q_pred[,1],col=4,lwd=2,lty=2)
    # lines(data$x_pred,q_pred[,2],col=4,lwd=2,lty=2)
    polygon(cbind(data$x_pred,rev(data$x_pred)),
            cbind(q_conf[,1],rev(q_conf[,2])),
            col=black_tr, border=NA)
    if (meth_type == 'gp') {
      polygon(cbind(data$x_pred,rev(data$x_pred)),
...
plot_all  <- function(legend=FALSE) {
  ...
  tiff(file=paste0(dir_out,case,'_sample_',model_tag,'.tiff'),
  ...
  tiff(file=paste0(dir_out,case,'_residuals_',model_tag,'.tiff'),
      width=reso, height=reso, compression="lzw")
  ...
  tiff(file=paste0(dir_out,case,'_predict_',model_tag,'.tiff'),
      width=reso, height=reso, compression="lzw")
  ...
  tiff(file=paste0(dir_out,case,'_predict_centered_',model_tag,'.tiff'),
      width=reso, height=reso, compression="lzw")
...
plot_shifts <- function(model_tags, true_shifts=NULL, 
  ...
  dir_out = paste0('simulation/',case,'/figures/')
  tiff(file=paste0(dir_out,case,'_compare_shifts.tiff'),
  ...
  for (model_tag in model_tags) {
    ...
    x = 1:nShift + 0.12*(imod-1) - 0.12*length(model_tags)/2
    if(imod == 1) {
    ...
    segments(x,qShift[,1],x,qShift[,5],lwd=4,col=imod)
    segments(x,qShift[,2],x,qShift[,4],lwd=6,col=imod)
    points(x,qShift[,3],pch=23,col=imod,cex=0.7)
  }
  # abline(v=1:nShift)
  grid()
  legend('topleft',legend=model_tags,lty=1,col=1:imod,ncol=3, lwd=5,
